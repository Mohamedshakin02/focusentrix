...
import { Mail, User, Lock, CreditCard, Shield, Eye, EyeOff, ShieldCheck, Zap, RefreshCw, HeadphonesIcon, Package, ArrowRight } from 'lucide-react'
...
export default function Checkout() {
    ...
    const handlePay = async (e) => {
        e.preventDefault();
        if (!form.email || !form.fullName || !form.password || !form.cardNumber) {
            return CustomizedToast.error("Please fill all billing fields");
        }

        try {
            const res = await axios.post("https://focusentrix-backend.onrender.com/api/auth/pro-signup", {
                name: form.fullName,
                email: form.email,
            ...
            CustomizedToast.success("Payment Successful! Welcome to Pro Version.", { autoClose: 10000 });
            navigate("/dashboard");
        } catch (err) {
            CustomizedToast.error(err.response?.data?.message || "Transaction failed");
        }
    };

    // Google Pro Signup Handler
    const googleProSignup = useGoogleLogin({
        onSuccess: async (tokenResponse) => {
            try {
                // 1. Fetch user info from Google API using the access token
                const userInfo = await axios.get(
                    "https://www.googleapis.com/oauth2/v3/userinfo",
                    { headers: { Authorization: `Bearer ${tokenResponse.access_token}` } }
                );

                // 2. Send details to your backend pro-signup route
                const res = await axios.post("https://focusentrix-backend.onrender.com/api/auth/pro-signup", {
                ...
                CustomizedToast.success("Google Payment Successful! Welcome to Pro.");
                navigate("/dashboard");
            } catch (err) {
                CustomizedToast.error(err.response?.data?.message || "Google Signup Failed");
            }
        },
        onError: () => CustomizedToast.error("Google Login Failed")
    });
        ...
        <>
            ...
            <div className="bg-[#0a0a0f] min-h-screen text-white container mx-auto px-4 sm:px-6 lg:px-30">
                ...
                <section className="w-full px-4 pb-12">
                    <form onSubmit={handlePay} className="max-w-6xl mx-auto bg-[#0e0b1e] border border-[#1e1535] rounded-3xl px-6 sm:px-10 py-8 flex flex-col gap-6">
                        ...
                        <div className="flex flex-col gap-7">
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="relative">
                                    ...
                                    <input type="email" name="email" placeholder="youremail@example.com" value={form.email} onChange={handleChange} className="w-full bg-[#13102a] border border-[#2a1a40] rounded-xl pl-11 pr-4 py-3.5 text-white text-sm placeholder-[#3d2060] focus:outline-none focus:border-[#9b59f5] transition-colors" />
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="relative">
                                    ...
                                    <input type="text" name="fullName" placeholder="John Doe" value={form.fullName} onChange={handleChange} className="w-full bg-[#13102a] border border-[#2a1a40] rounded-xl pl-11 pr-4 py-3.5 text-white text-sm placeholder-[#3d2060] focus:outline-none focus:border-[#9b59f5] transition-colors" />
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="relative">
                                    ...
                                    <input type={showPassword ? 'text' : 'password'} name="password" placeholder="Create password" value={form.password} onChange={handleChange} className="w-full bg-[#13102a] border border-[#2a1a40] rounded-xl pl-11 pr-12 py-3.5 text-white text-sm placeholder-[#3d2060] focus:outline-none focus:border-[#9b59f5] transition-colors" />
                                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#5a4a7a] hover:text-white transition-colors">
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="relative">
                                    ...
                                    <input type="text" name="cardNumber" placeholder="1234 5678 9012 3456" value={form.cardNumber} onChange={handleCardInput} className="w-full bg-[#13102a] border border-[#2a1a40] rounded-xl pl-11 pr-4 py-3.5 text-white text-sm placeholder-[#3d2060] focus:outline-none focus:border-[#9b59f5] transition-colors tracking-widest" />
                                </div>
                            </div>
                            ...
                            <div className="bg-[#13102a] border border-[#2a1a40] rounded-2xl px-6 py-5 flex flex-col md:flex-row justify-between ">
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-full bg-[#1e1040] border border-[#3d2060] flex items-center justify-center flex-shrink-0">
                                ...
                                <div className="text-right w-50 md:w-auto d-flex jus flex-shrink-0 mt-4 md:mt-0 relative">
                            ...
                            <button type="submit" className="w-full text-white font-semibold py-3.5 mt-2 rounded-xl bg-gradient-to-b from-[#7A34F0] via-[#6229C1] to-[#501CA0] shadow-[0_4px_12px_rgba(123,44,191,0.4)] hover:shadow-[0_6px_18px_rgba(123,44,191,0.6)] flex items-center justify-center gap-2">
                        ...
                        <button type="button" onClick={() => googleProSignup()} className="w-full flex items-center justify-center gap-3 bg-white hover:bg-gray-100 text-black font-bold py-4 rounded-2xl transition-colors">
