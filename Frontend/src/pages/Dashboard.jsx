...
// dashboard main page components
export default function Dashboard() {
  ...
  // configuration state(when user is adjusting the settings)
  ...
  // when track index changes, load and play the new track
  useEffect(() => {
    if (!audioRef.current) return
    const src = tracks[trackIndex].src
    if (!src) return // no file imported yet, skip
    audioRef.current.src = src
    audioRef.current.load()
    if (musicPlaying) audioRef.current.play()
  }, [trackIndex])

  // sync volumes slider with audio element
  useEffect(() => {
    if (audioRef.current) audioRef.current.volume = volume / 100
  }, [volume])

  // play or pause when musicPlaying state changes
  useEffect(() => {
    if (!audioRef.current) return
    const src = tracks[trackIndex].src
    if (!src) return
    if (musicPlaying) {
      ...
      audioRef.current.play().catch(() => {}) // catch autoplay policy errors silently
    } else {
  ...
  // streak data is here for the current streak card
  ...
  return (
    ...
    <div className="bg-[#0a0a0f] min-h-screen text-white">
      ...
      <div className="w-full px-10 pt-10 pb-24">
        ...
        {/* row above the stat cards that welcomes user*/}
        <div className="mb-6">
        ...
        <div className="flex gap-4 mb-6 items-stretch">
          ...
          <div className="bg-[#0e0b1e] border border-[#1e1535] rounded-2xl px-6 py-5 flex flex-col items-center gap-3 w-56 flex-shrink-0 relative">
            ...
            {/* settings gear which opens configuration panel */}
            <button
              onClick={() => setShowConfig(!showConfig)}
              className="absolute top-4 left-4 text-[#5a4a7a] hover:text-[#9b59f5] transition-colors duration-200"
            >
        ...
        <div className="grid grid-cols-[1fr_220px] gap-4">
          ...
          <div className="flex flex-col gap-4">
            ...
            {/* camera conitoring header with p text*/}
            <div className="bg-[#0e0b1e] border border-[#1e1535] rounded-2xl overflow-hidden">
              ...
              <div className="flex border-t border-[#1e1535]">
                <button onClick={() => setPaused(!paused)}
                  className="flex-1 py-3 text-sm font-semibold text-white hover:bg-[#1e1535] transition-colors duration-200 border-r border-[#1e1535]">
                  {paused ? 'Resume' : 'Pause'}
                </button>
                <button onClick={() => setMonitoring(!monitoring)}
                  className={`flex-1 py-3 text-sm font-semibold transition-colors duration-200 border-r border-[#1e1535]
                    ${monitoring ? 'bg-[#9b59f5] text-white' : 'text-[#8a7aaa] hover:bg-[#1e1535]'}`}>
                  {monitoring ? 'Monitoring On' : 'Monitoring Off'}
                </button>
                <button className="flex-1 py-3 text-sm font-semibold text-white hover:bg-[#1e1535] transition-colors duration-200">
                  Settings
                </button>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="bg-[#0e0b1e] border border-[#1e1535] rounded-2xl px-5 py-5 flex flex-col gap-4">
              ...
              {/*alert if there is no file found*/}
